...
import { db } from './firebase'; // Make sure you have firebase.js file with db export
...
const App = () => {
    ...
    const [isTravelManagementOpen, setIsTravelManagementOpen] = useState(false);
    ...
    const addReservation = async (newReservation) => {
        try {
            ...
            setReservations(prev => [{ ...newReservation, id: docRef.id }, ...prev]);
    ...
    const addTour = async (newTour) => {
        try {
            ...
            setTours(prev => [...prev, { ...newTour, id: docRef.id }]);
    ...
    const addInsurance = async (newPolicy) => {
        try {
            const docRef = await addDoc(collection(db, "insurances"), newPolicy);
            setInsurances(prev => [...prev, { ...newPolicy, id: docRef.id }]);
    ...
    const handleDeleteReservation = async (idToDelete) => {
        const confirmation = window.confirm(`Are you sure you want to delete reservation ${idToDelete}?`);
        if (confirmation) {
            try {
    ...
    const handleUpdateInsurance = async (updatedPolicy) => {
        try {
            const insuranceRef = doc(db, "insurances", updatedPolicy.id);
            await updateDoc(insuranceRef, updatedPolicy);
