...
// New: State for Sales Invoice Form
 const [salesInvoiceForm, setSalesInvoiceForm] = useState({
  ...
  // NEW: Firestore listener for Tasks
useEffect(() => {
    let unsubscribe;
    if (isAuthReady && userId) {
        if (!db) {
...
case 'taskManagement':
        ...
        default:
            return <div>Select a module from the navbar.</div>;
    } // This closes the 'switch' statement
}; // This closes the 'renderContent' function
...
const handleSubmitTask = async (e) => {
    ...
    try {
        ...
        resetTaskForm();
        setActiveTab('taskManagement');
    } catch (err) {
        console.error("Error saving task:", err);
        setError(`Failed to save task: ${err.message || err.toString()}`);
...
const handleEditTask = useCallback((task) => {
    ...
    setActiveTab('taskManagement'); // Keep on the same tab, but the form will appear
}, []);
