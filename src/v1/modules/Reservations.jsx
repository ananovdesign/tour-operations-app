...
import { 
  Search, Plus, Eye, Edit3, FileText, Loader2, User, Calendar
} from 'lucide-react';
...
const Reservations = ({ lang = 'bg' }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(resRef, (snapshot) => {
      ...
      // СОРТИРАНЕ: От най-малък към най-голям номер на резервация
      const sortedData = data.sort((a, b) => {
        const numA = parseInt(a.reservationNumber) || 0;
        const numB = parseInt(b.reservationNumber) || 0;
        return numA - numB;
      });
  ...
  if (loading) return (
    <div className="flex h-64 flex-col items-center justify-center space-y-4">
      ...
      <p className="text-slate-400 font-black italic uppercase">Сортиране на базата...</p>
    </div>
  );
    ...
    <div className="space-y-6 animate-in fade-in duration-500 font-sans">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="relative w-full md:w-96">
          ...
          <input 
            type="text"
            placeholder="Search by Lead Guest or Res. Number..."
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button className="bg-blue-600 hover:bg-blue-700 text-white p-3 rounded-2xl shadow-lg transition-all flex items-center gap-2 font-black uppercase text-[10px] tracking-tighter">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20">
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Number</th>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
              {filteredReservations.map((res) => (
                <tr key={res.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-6 py-5 font-black text-blue-600 dark:text-blue-400">#{res.reservationNumber}</td>
                  <td className="px-6 py-5 text-sm font-bold truncate max-w-[150px]">{res.hotel}</td>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      {/* Използваме paymentStatusColor от базата, ако съществува */}
                      <span className={`text-[10px] font-black uppercase tracking-tight px-2 py-0.5 rounded-lg w-fit ${
                  ...
                  <td className="px-6 py-5 font-black text-sm text-right text-slate-900 dark:text-white">
                    {res.profit?.toFixed(2)} <span className="text-[10px] text-slate-400">BGN</span>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="p-2 hover:bg-blue-50 text-blue-500 rounded-xl border border-slate-100 dark:border-slate-800">
                        <Eye size={14} />
                      </button>
                      <button className="p-2 hover:bg-emerald-50 text-emerald-500 rounded-xl border border-slate-100 dark:border-slate-800">
                        <Edit3 size={14} />
                      </button>
                      <button className="p-2 hover:bg-purple-50 text-purple-500 rounded-xl border border-slate-100 dark:border-slate-800">
                        <FileText size={14} />
                      </button>
                    </div>
                  </td>
