...
import './InvoicePrint.css'; // Import the CSS we just created
...
const InvoicePrint = ({ invoiceData }) => {
  // Trigger the print dialog as soon as the component loads
  useEffect(() => {
    // A small delay ensures all content and images are rendered before printing
  ...
  const fullClientAddress = [invoiceData.clientAddress, invoiceData.clientCity, invoiceData.clientPostCode].filter(Boolean).join(', ');
   ...
   return (
    <div className="print-preview-container">
      ...
      <table className="details-table">
        ...
        <tbody>
          {invoiceData.products.map((item, index) => {
            const priceEUR = (item.price || 0) / EUR_TO_BGN_RATE;
            const itemSumEUR = (item.lineTotal || 0) / EUR_TO_BGN_RATE;
            return (
              <tr key={index}>
                <td>{index + 1}</td>
                ...
                <td>{item.price.toFixed(2)} ({priceEUR.toFixed(2)} EUR)</td>
                <td>{item.lineTotal.toFixed(2)} ({itemSumEUR.toFixed(2)} EUR)</td>
              </tr>
            );
          })}
        </tbody>
      ...
      <div className="summary-section">
        ...
        <div className="right-col">
          <table className="summary-table">
            <tbody>
                <tr>
                    <td className="label">Данъчна основа {invoiceData.products[0]?.vatRate || 20}%:</td>
                    <td>{invoiceData.totalAmount.toFixed(2)} ({totalBasePriceEUR.toFixed(2)} EUR)</td>
                </tr>
                <tr>
                    <td className="label">ДДС {invoiceData.products[0]?.vatRate || 20}%:</td>
      ...
      </div> // <-- ADD THIS MISSING LINE
    </div> // This closes "print-preview-container"
  );
};
